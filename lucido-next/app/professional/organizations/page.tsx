...
export default function OrganizationsPage() {
  ...
  const orgsWithMembers = userOrgs.map((userOrg) => {
    ...
    return {
      ...
      members,
      patients: patients.filter((p) => p.organizationId === userOrg.organization!.id),
    };
  }).filter((o) => o !== null);
    ...
    <div className="space-y-6">
      ...
      {orgsWithMembers.map((org) => (
        <div key={org.id} className="rounded-lg bg-white p-6 shadow-sm border border-gray-200">
          ...
          <div>
            <h3 className="mb-3 font-semibold text-gray-900">Profissionais conectados</h3>
          ...
          <div className="mt-6">
            <h3 className="mb-3 font-semibold text-gray-900">Pacientes da organização</h3>
            {org.patients.length === 0 ? (
              <p className="text-sm text-gray-600">Nenhum paciente vinculado.</p>
            ) : (
              <div className="grid gap-3 md:grid-cols-2">
                {org.patients.map((patient) => (
                  <div key={patient.id} className="rounded-md border border-gray-200 p-3">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-semibold text-gray-900">{patient.name}</p>
                        <p className="text-xs text-gray-600">{patient.diagnoses.join(", ")}</p>
                      </div>
                      <a
          ...
          {org.role === "ADMIN" && (
            <div className="mt-6 grid gap-3 md:grid-cols-2">
              <button className="rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700 transition border border-indigo-700">
                Conectar profissional da rede a um paciente
              </button>
              <button className="rounded-md bg-gray-200 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-300 transition border border-gray-300">
                Gerenciar pacientes da organização
              </button>
            </div>
          )}
        </div>
